...
| board-over-temperature | 0 | Board temperature in tenths of a degree C | CAN address of the board | Any drivers on that board that use external mosfets or do not monitor the temperature of internal mosfets are disabled until the temperature drops below the error threshold (before the event is raised). Pause print without running pause.g and inform user via message box | Error |
