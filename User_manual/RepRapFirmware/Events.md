...
| board-over-temperature | 0 | Board temperature in tenths of a degree C | CAN address of the board | Before the event is raised, any drivers on that board that use external mosfets or do not monitor the temperature of internal mosfets are disabled. They will be re-enabled when the temperature drops below the warning threshold. The default action is to pause the job without running pause.g and inform the user via a message box | Error |
