...
If you don't specify a CAN address then RRF assumes that you are referring to a pin on the main board.

Occasionally a pin may be used in two different modes. Currently this applies only to the `temp0` pin of the TOOL1LC board, which can use two dfferent types of analog-to-digital converter. The secondary mode can be selected by prefixing the pin name with `*`.
