...
1. Printing with debugging enabled. If debugging is enabled, RepRapFirmware sends large quantities of debugging data to the USB port, and waits a reasonable time to allow the host program to pick them up. This can greatly slow down the processing of GCode commands. Note that Repetier Host enabled debug automatically in its default configuration, because Repetier firmware appropriates the M111 debugging command for its own use.
