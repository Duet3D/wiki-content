...
The uncalibrated microcontroller temperature on the Duet 2 (WiFi and Ethernet) should be within 13C of the true temperature. For the Duet 3 MB6HC it could be up to 39C out. The microcontroller used in Duet 3 Mini does not have a working MCU temperature sensor.
...
1. As quickly as possible (before the Duet has time to heat up), connect to it using Duet Web Control and read the temperature from the CPU Temp. box on the Machine Control page. Call this Tc.
...
1. Add that M912 command to your config.g file. It doesn't matter where, but I put it near the end, before the M501 command and the T0 command.
