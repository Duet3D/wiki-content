...
In RRF versions prior to 3.5, M292 may ony be used to acknowledge blocking message boxes. RRF 3.5 and later allow M292 to be used to cancel non-blocking messages boxes too, provided that the correct sequence number is specified.
...
**P"nnn" parameter for "dht-humidity", "bme280-pressure" and "bme280-humidity**
...
* **Tn** Encoder type: 0=none, 1=linear composite encoder (RRF 3.5 only), 2=quadrature motor shaft encoder, 3=Duet magnetic motor shaft encoder using AS5047D (RRF 3.5 only)
...
* **En** (optional) The number of pulses generated by the rotary encoder per detent. Typical values are 2 and 4. Negative values (e.g. -2 and -4) reverse the encoder direction.
...
From RepRapFirmware 3.2.0 and later, more granular logging is available. There are three log levels, and no logging. Each line in the log will have the log level of that message added after the timestamp.
