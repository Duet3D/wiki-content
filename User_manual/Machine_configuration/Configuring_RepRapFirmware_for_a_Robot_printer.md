...
**B"orientationType=full|zaxis|verticalRot|no"**
...
* verticalRot means endpoint's orientation, i. e. the Z axis is vertical up or down and rotated by a specific angle with respect to the base X axis
...
* 4 axis palletized RRRpR with 4th actuator can be set to verticalRot, because the endpoint rotation with respect to rotation about Z axis can be controlled by firmware. This allows e. g. fixed probe offset for mesh compensation measurement by setting the endpoint to be parallel to the X axis.
