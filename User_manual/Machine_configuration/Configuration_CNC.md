...
In RepRapFirmware v3.3b2 and later, use the [M950](/User_manual/Reference/Gcodes/M950) command to configure which outputs to use for spindle motor control. Define the spindle as a tool with [M563](/User_manual/Reference/Gcodes/M563).

In earlier RepRapFirmware version, use the M453 command to put the firmware in CNC mode AND to configure which outputs to use for spindle motor control. You need to define at least one tool using [M563](/User_manual/Reference/Gcodes/M563), but it can have zero associated heaters and zero extruder drives.
