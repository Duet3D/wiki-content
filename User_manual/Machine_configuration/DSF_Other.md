...
The link between Duet and SBC requires an extra GPIO pin so that the Duet can flag when it is ready to exchange data. This port defaults to [pin 25](https://pinout.xyz/pinout/pin22_gpio25#) on the Raspberry Pi. Furthermore DSF uses `libgpiod` to access this pin.

*The following steps can be skipped if a Raspberry Pi is used.*
